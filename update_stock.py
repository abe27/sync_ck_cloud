...
def main():
    sql = f"UPDATE TXP_CARTONDETAILS SET RECLOCATE=SHELVE,SIDTE=NULL,SINO=NULL,SIID=NULL WHERE STOCKQUANTITY > 0 AND SIDTE IS NOT NULL"
    Oracur.execute(sql)
    sql = f"UPDATE TXP_CARTONDETAILS SET RECLOCATE=SHELVE,SIDTE=UPDDTE,SINO='TIMVOUT',SIID='SKTSYS',STOCKQUANTITY=0 WHERE SHELVE='S-CK1'"
    Oracur.execute(sql)
