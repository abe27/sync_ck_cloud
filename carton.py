...
async def main():
    sql = f"SELECT INVOICENO,PARTNO,LOTNO,RUNNINGNO,RVMANAGINGNO,STOCKQUANTITY,SHELVE,CASE WHEN PALLETKEY IS NULL THEN '-' ELSE PALLETKEY END PALLETKEY,CASE WHEN CASEID IS NULL THEN '-' ELSE CASEID END CASE_ID,PALLETNO transfer_out,RECEIVINGQUANTITY  FROM TXP_CARTONDETAILS WHERE IS_CHECK=0 ORDER BY PARTNO,RUNNINGNO FETCH FIRST 50 ROWS ONLY"
